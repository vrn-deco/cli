...
export default function factory(...args: CreateActionArgs): CreateAction {
  return new CreateAction(...args)
}
...
export class CreateAction extends Action<CreateArguments, CreateOptions> {
  ...
  async initialize(): Promise<void> {
     ...
     * 6.询问选择一个模板
     */
    let [folderName, baseDirectory] = this._args
    this.folderName = folderName
    this.baseDirectory = path.resolve(process.cwd(), baseDirectory ?? this.DEFAULT_BASEDIRECTORY)
